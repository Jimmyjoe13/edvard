...
window.EdvardPDF = {
    generate: (state) => {
        ...
        doc.text("Fiche de Personnage - Projet Edvard", 105, 20, { align: "center" });
        ...
        doc.text(`Spécialisation : ${formatString(state.specialization)}`, rightCol, y);
        y += 8;
        ...
        const bgLines = doc.splitTextToSize(`Histoire: ${state.lore.background || "Aucune"}`, 170);
        doc.text(bgLines, leftCol, y);
        y += (bgLines.length * 5) + 5;

        const appLines = doc.splitTextToSize(`Apparence: ${state.lore.appearance || "Non décrite"}`, 170);
        doc.text(appLines, leftCol, y);

        // Footer
        doc.setFontSize(8);
        doc.setTextColor(150);
        doc.text("Généré sur Projet Edvard - Le JDR Magi-Tech", 105, 280, { align: "center" });
