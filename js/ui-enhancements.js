...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.dice-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            setTimeout(() => {
                ...
                if(historyContainer.children.length > 5) historyContainer.lastChild.remove();
            }, 300);
        });
    });
    ...
    if (document.querySelector('.character-creation')) {
        ...
        function updatePreview(state) {
            ...
            document.getElementById('prev-race').textContent = state.race ? capitalize(state.race) : '-';
            document.getElementById('prev-spec').textContent = state.specialization ? capitalize(state.specialization) : '-';
            document.getElementById('prev-hp').textContent = state.derived ? state.derived.hp : '-';
            ...
            Object.keys(statsToDisplay).forEach(k => {
               ...
               const mod = window.EdvardUtils ? window.EdvardUtils.calculateModifier(val) : Math.floor((val - 10) / 2);
               const modStr = window.EdvardUtils ? window.EdvardUtils.formatModifier(mod) : (mod >= 0 ? `+${mod}` : mod);
